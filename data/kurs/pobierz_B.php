<?php
echo STAR.'Średnie kursy walut dodatkowych';
$link = @file_get_contents('http://www.nbp.pl/kursy/kursyb.html');
if(!$link) {
	echo FAIL;
	return FALSE;
}

$last = @file_get_contents('./B_last_link.txt');
...
file_put_contents('./B_last_link.txt', $lnk);
unset($lnk);
...
file_put_contents('./B_kursy.txt', serialize($waluta));
file_put_contents('./archiwum/B_'.date('j.m.Y', strtotime($waluta['i_n_f_o']['data'])).'.txt', serialize($waluta));
